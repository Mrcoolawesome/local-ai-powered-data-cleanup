...
- [~] **Zoom spike, stage 2 (Zoom Linux Meeting SDK join + `StartAppShare`):** SDK obtained; code compiles/links/runs against it; auth round-trip verified against Zoom's real servers with placeholder credentials. Remaining: run with real SDK Key/Secret + a test meeting to confirm the actual join+share — see [`/spikes/zoom-presentation-bot/README.md`](../spikes/zoom-presentation-bot/README.md). Outcome once that final step runs: proceed with native design, or move to a fallback.
