...
| Target schemas | Mirror/import from the existing "Lighthouse" `DataTransformer` schema system (`crunchwrap_supreme/.../data_transformer/schemas.ex`) where it already defines an entity (jobs, invoices, estimates) | Discovered via the HouseCall Pro scraper README — Lighthouse already has canonical target schemas for some entities. Redefining them independently would create drift. Exact integration mechanism (shared package? synced JSON? read-only reference?) is an open item pending access to that codebase — see [09-roadmap.md](./09-roadmap.md) Phase 0. |
