...
- [x] **Zoom spike, stage 2 (Zoom Linux Meeting SDK join + `StartAppShare`):** bot joined a real, live meeting (`MEETING_STATUS_INMEETING` confirmed) using real credentials. `StartAppShare` itself returned `SDKERR_NO_PERMISSION` — a Zoom account/meeting "who can share" setting, not a code or architecture problem. **Decision: proceed with the native design** — see [07-zoom-bot.md](./07-zoom-bot.md) for the fix (adjust sharing permissions or join with host/co-host privileges) and full result.
