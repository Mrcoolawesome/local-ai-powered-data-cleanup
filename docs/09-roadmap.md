...
- [x] `TargetSchema` + `CleaningRule` UI (create/edit), seeded from the concrete column sets in [10-target-schema-reference.md](./10-target-schema-reference.md). Started with Contacts/Jobs/Invoices; **all 8 entity types now built** (Contacts, Jobs, Invoices, Estimates, Pricebook - Services, Pricebook - Materials, Equipment, Memberships) once the project owner provided their own real migration template covering every sheet — not built speculatively, driven by that real template. Verified for real: created a `TargetSchema` from all 8 templates through the actual app and confirmed every column/required-count matched the source spreadsheet exactly.
