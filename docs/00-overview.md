...
| Zoom bot | Proceed with the native Zoom Linux SDK + Xvfb + `StartAppShare` design (no fallback needed) | Spiked end-to-end against a real, live Zoom meeting: the bot authenticated, joined, and reached `MEETING_STATUS_INMEETING`. The share call itself hit `SDKERR_NO_PERMISSION` — a Zoom account/meeting sharing-permission setting, not a technical limitation — see [07-zoom-bot.md](./07-zoom-bot.md) for the fix and full spike results. |
