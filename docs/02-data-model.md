...
**TargetSchema**
A user-defined (or Lighthouse-mirrored — see [00-overview.md](./00-overview.md)) target shape for a given entity type (Jobs, Invoices, Estimates, Contacts, …). Versioned (`version` int) so a change to the schema doesn't silently reinterpret old audit history. Fields: `entityType`, `columns` (JSON: name, type, required flag, description), `sourceOfTruth` (`user_defined` | `lighthouse_mirror`).
